#!/usr/bin/env python3
"""
Shopee Feed Auto-Poster: Step 3 (Facebook Module)
Workflow Runner:
1. Read 'temp/shopee_payload.json'.
2. Post product image + Facebook AI caption to Facebook Page Feed.
3. Automatically post the official Shopee affiliate link into the 1st comment.
4. Output 'POST ID :' for GitHub Actions tracking and debugging.
5. Update 'post_results.facebook' inside 'temp/shopee_payload.json'.
"""
...
def post_to_facebook_page(page_id: str, page_token: str, caption: str, image_url: str, affiliate_link: str):
    ...
    diikuti dengan menghantar pautan Shopee ke ruangan komen pertama.
    """
    ...
    for attempt in range(2):
        try:
            if img_bytes:
                files = {"source": ("shopee_product.jpg", img_bytes, "image/jpeg")}
                photo_payload = {
                    "caption": caption,
                    "published": "true",
    ...
    # 3. Masukkan Pautan Affiliate Shopee ke Ruangan Komen Pertama
    ...
    if clean_link and target_post_id:
        try:
            ...
            comment_text = f"🛒 Dapatkan di Shopee sekarang👇\n{clean_link}"
            comment_payload = {
...
def run_facebook_posting():
    print("\n" + "=" * 70)
    print("📘 [START] MEMULAKAN HANTARAN SHOPEE KE FACEBOOK PAGE FEED")
    print("=" * 70)
