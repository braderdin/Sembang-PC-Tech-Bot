#!/usr/bin/env python3
"""
Shopee Feed Auto-Poster: Step 4, 5, 6, 7 & 8 (Audit & Commit Module)
Workflow Runner:
...
4. Step 5: Commit product ID to Upstash Redis (Key: 'shopee:product:<id>', TTL 30 Days).
5. Step 6: Commit product title to Upstash Vector DB (ID: 'sp_<id>', Platform: 'shopee').
6. Step 7: Update Supabase table (shopee_status_used = true).
7. Step 8: Clean up temporary payload file 'temp/shopee_payload.json'.
"""
...
from src.shopee_supabase import mark_shopee_product_as_used
...
def run_audit_and_commit():
    print("\n" + "=" * 70)
    print("📊 [START] MEMULAKAN AUDIT TELEGRAM & KOMIT TRANSAKSI SHOPEE")
    print("=" * 70)
    ...
    product_name = str(payload.get("product_name") or "Produk Shopee").strip()
    ...
    if vector_ok:
        print(f"✅ Vector: Embedding 'sp_{product_id}' (Metadata: 'shopee') berjaya direkodkan.")
    else:
