#!/usr/bin/env python3
"""
Lazada Auto-Poster Pipeline: Comprehensive Local Test Runner
...
def check_environment_variables():
    ...
    print_banner("STEP 0: PRE-FLIGHT ENVIRONMENT & CONFIG CHECK (LAZADA)", YELLOW)
...
def run_full_pipeline_test():
    ...
    print_banner("🧪 LAZADA FEED AUTO-POSTER: END-TO-END PIPELINE TEST", BOLD + CYAN)
    ...
    try:
        from bin.run_lazada_prepare_and_generate import run_preparation_and_generation
        ...
        if PAYLOAD_FILE.exists():
            ...
            print(f"   📦 Nama     : {payload.get('product_name') or payload.get('title')}")
            print(f"   💰 Harga    : RM {payload.get('price', 0):.2f}" if payload.get('price') else "   💰 Harga    : Tawaran Berbaloi")
            print(f"   🔗 Pautan   : {payload.get('affiliate_link')}")
            step_results["Step 1 & 2 (Prepare & Generate)"] = "SUCCESS"
        else:
            raise FileNotFoundError("Fail temp/lazada_payload.json tidak wujud selepas Step 1 & 2.")
    ...
    try:
        from bin.run_lazada_post_facebook import run_facebook_posting
    ...
    try:
        from bin.run_lazada_post_threads import run_threads_posting
        run_threads_posting()
    ...
    try:
        from bin.run_lazada_post_instagram import run_instagram_posting
    ...
    try:
        from bin.run_lazada_post_bluesky import run_bluesky_posting
        run_bluesky_posting()
    ...
    try:
        from bin.run_lazada_audit_and_commit import run_audit_and_commit
    ...
    print_banner("📊 LAPORAN KEPUTUSAN UJIAN KESELURUHAN (LAZADA)", BOLD + GREEN)
