#!/usr/bin/env python3
"""
Shopee Feed Auto-Poster: Step 3 (Threads Module)
Workflow Runner:
1. Read 'temp/shopee_payload.json'.
2. Validate and ensure Threads character limit <= 500 characters.
3. Step A: Create Threads Media Container via Graph API.
4. Step B: Publish container to Threads profile.
5. Output 'POST ID :' for GitHub Actions tracking.
6. Update 'post_results.threads' inside 'temp/shopee_payload.json'.
"""
...
def post_to_threads(user_id: str, access_token: str, caption: str, image_url: str = ""):
    ...
    try:
        # =====================================================================
        # LANGKAH 1: Cipta Media Container (Auto-Retry jika pelayan sibuk)
        # =====================================================================
        print(f"🧵 [THREADS STEP A] Membina Media Container (Saiz Teks: {len(safe_caption)}/500 aksara)...")
        ...
        # Beri masa pelayan Meta memproses CDN imej sebelum diterbitkan
    ...
    return False, "Gagal menerbitkan ke Threads."


def run_threads_posting():
    print("\n" + "=" * 70)
    print("🧵 [START] MEMULAKAN HANTARAN SHOPEE KE META THREADS FEED")
    print("=" * 70)
    ...
    if not user_id or not access_token:
        print("⚠️ [THREADS SKIP] Kunci THREADS_USER_ID atau THREADS_ACCESS_TOKEN tiada dalam env. Langkau.")
    ...
    # 3. Lakukan hantaran ke Threads Feed
    ...
    if ok:
        ...
        print(f"\n🎉 [THREADS SUCCESS] Hantaran berjaya dipos ke Threads!")
