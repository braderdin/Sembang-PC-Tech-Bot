#!/usr/bin/env python3
"""
Lazada Bluesky AI Persona Engine (Brader Din Style - AT-Protocol Optimized)
Sembang PC & Tech Ecosystem (100% Dynamic OpenRouter & Glitch-Proof)
Features:
- Micro-Blogging for Bluesky Feed (Strict Hard Limit <= 295 Characters)
- Auto-Calculated Safe Text Window (100 - 160 Chars Body) for Direct Link & Hashtag Injection
- Strict Anti-Glitch & Malay Language Anchor Guardrails
- 3-Attempt Auto-Retry Loop with Resilient Fallback Engine
"""
...
class LazadaBlueskyAIPersona:
    """Enjin AI Persona Bluesky khusus untuk hantaran produk Lazada pantas (Hard Limit <= 295 Chars)."""
    ...
    def generate_caption(self, product_data: Dict[str, Any]) -> Tuple[bool, str]:
        """
        Menjana kapsyen penuh Bluesky Feed bersama pautan Lazada (Maksimum TEGAS <= 295 aksara).
        Memulangkan: (success_bool, full_bluesky_text)
        """
        raw_title = str(
            product_data.get("lazada_product_name")
            or product_data.get("title")
            or product_data.get("product_name")
            or "Gajet Pilihan"
        ).strip()
        clean_title = re.sub(r'\s+', ' ', raw_title)[:55].strip()
        brand = product_data.get("lazada_brand") or product_data.get("brand") or "Pilihan Ramai"
        price = product_data.get("lazada_price") or product_data.get("price") or ""
        aff_link = str(
            product_data.get("lazada_affiliate_link")
            or product_data.get("affiliate_link")
        ...
        link_part = f"\n\n🛒 Lazada: {aff_link}" if aff_link else ""
        hashtag_part = "\n\n#SembangPCTech #LazadaMY"
        footer = f"{link_part}{hashtag_part}"

        # Had maksimum ketat keseluruhan Bluesky ialah 300 aksara. Sasarkan 290 aksara untuk zon selamat.
        ...
        if not self.base_url or not self.model or not self.api_key:
            print("⚠️ [LAZADA BLUESKY AI WARN] Kunci OpenRouter tidak lengkap, menggunakan kapsyen sandaran.")
...
"""
        ...
        for attempt in range(3):
            try:
                print(f"🤖 [LAZADA BLUESKY AI] Menjana kapsyen Bluesky (Percubaan {attempt + 1}/3)...")
                ...
                if res.status_code == 200:
                    ...
                    if "choices" in data and len(data["choices"]) > 0:
                        ...
                        if is_valid_bluesky_caption(final_body, min_len=25):
                            ...
                            print(f"✅ [LAZADA BLUESKY AI SUCCESS] Kapsyen Bluesky berjaya dijana ({len(full_post)}/295 aksara).")
                            return True, full_post
                        else:
                            print(f"⚠️ [LAZADA BLUESKY AI GLITCH] Teks gagal tapisan kualiti pada percubaan {attempt + 1}. Mencuba semula...")
                else:
                    print(f"⚠️ [LAZADA BLUESKY AI HTTP ERROR] HTTP {res.status_code}: {res.text}")

            except Exception as e:
                print(f"⚠️ [LAZADA BLUESKY AI EXCEPTION - ATTEMPT {attempt + 1}]: {str(e)}")

        print("🛡️ [LAZADA BLUESKY AI FALLBACK] Mengaktifkan kapsyen Bluesky sandaran bersih.")
...
lazada_bluesky_ai = LazadaBlueskyAIPersona()
bluesky_ai = lazada_bluesky_ai