#!/usr/bin/env python3
"""
Lazada Telegram Audit & Notification Module
Sembang PC & Tech Ecosystem (Telegram Bot API)
Features:
- Dual-Message Audit Report (Photo Status Card + Full AI Captions Preview)
- Safety Gatekeeper: Validates at least 1 successful platform post before database commit
- Smart HTML Escaping & Telegram 4096-char chunking engine
"""
...
def send_lazada_audit_report(payload: Dict[str, Any]) -> Tuple[bool, str]:
    """
    Menghantar laporan lengkap aliran kerja Lazada ke saluran Telegram Audit:
    ...
    prod_name = html.escape(str(payload.get("product_name", payload.get("title", "Produk Lazada"))))
    ...
    pic_url = payload.get("picture_url", payload.get("image_url", ""))
    post_results = payload.get("post_results", {})
...
# Alias keserasian ke belakang
send_audit_report = send_lazada_audit_report